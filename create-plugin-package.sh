...
# Coinsub Plugin Package Creator
...
cd "$PACKAGE_DIR"
zip -r "../coinsub.zip" . -x "*.DS_Store" "*.git*"
cd ..
...
echo "📁 Package: coinsub.zip"
echo "📋 Size: $(du -h coinsub.zip | cut -f1)"

# Copy to Downloads folder
DOWNLOADS_DIR="$HOME/Downloads"
if [ -d "$DOWNLOADS_DIR" ]; then
    echo "📥 Copying to Downloads folder..."
    cp coinsub.zip "$DOWNLOADS_DIR/"
    echo "✅ Also saved to: $DOWNLOADS_DIR/coinsub.zip"
else
...
echo "1. Upload coinsub.zip to WordPress"
